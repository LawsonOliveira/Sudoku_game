...
from Data import *
import pygame
from pygame.constants import K_0, K_BACKSLASH, K_BACKSPACE, KEYDOWN

from Grid.Check_solution import check_solutions,is_full
...
NUMBERS_FONT = pygame.font.SysFont('comicsans', 50)
COMMENTS_FONT = pygame.font.SysFont('comicsans',18)
TIME_FONT = pygame.font.SysFont('Arial',35)
NEW_GAME_FONT=pygame.font.SysFont('comicsans', 40)
...
##images
#image du crayon pour faire le bouton notes
...
WINDOW = pygame.display.set_mode((WINDOW_WIDTH, WINDOW_HEIGHT))  # crée la fenêtre générale
...
def draw_current_case_selected(grid,current_case_selected_i, current_case_selected_j):
    ...
    for k in range(3):
        for l in range(3):
            cases_a_colorier_gris.append((3*big_coord[0]+k,3*big_coord[1]+l)) #on ajoute également à cette liste toutes les cases qui sont dans le même carré 9x9 que la case sélectionnée
        ...
        for j in range(n):
            if grid[i][j] != 0:
                ...
                if (i,j) in original_coordinates:
                    ...
                    number_to_draw = NUMBERS_FONT.render(str(grid[i][j]), 1, BLUE)
                    WINDOW.blit(number_to_draw,
                                (X+X_NUMBER_OFFSET, Y+Y_NUMBER_OFFSET))
...
def draw_comments(comments_grid): #dessine les notes (contenus dans la liste de liste(=grille) de set() comments_grid) en petit et gris dans les cases
    offset_comments = {1 : (7,5) , 2 : (22,5) , 3 : (38,5) , 
                        4 : (7,20) , 5 : (22,20) , 6 : (38,20), 
                        7 : (7,35) , 8 : (22,35) , 9 : (38,35)}
    for i in range(9):
        for j in range(9):
            X, Y = j*BLOCK_SIDE, i*BLOCK_SIDE
...
def draw_new_game_button(): #dessine le bouton nouvelle partie qui remet tous les paramètres à l'état initial (timer,mode note,mode pause) et affiche une nouvelle grille de jeu
    ...
    WINDOW.blit(text,(490+30,180+20))
...
def draw_quit_button(): #dessine le bouton quiiter qui permet de quitter la partie
    ...
    WINDOW.blit(quit_text,(490+80,270+10))
...
def draw_show_button(): #dessine le bouton révéler qui permet d'afficher la solution à la grille initiale 
    ...
    WINDOW.blit(show_text,(490+80,350+10))
...
def draw_window(grid, current_case_selected_i, current_case_selected_j,original_coordinates,total_time, comments_grid,comments_activated, in_pause,game_won,music_activated):
    WINDOW.fill(WHITE)
...
# quand on appuie sur une des flèches du clavier, fait un update des fonctions
def update_current_case_selected(event, current_case_selected_i, current_case_selected_j):
    if event.type == pygame.KEYDOWN:  # remarque si on a appuyé sur une touche du clavier
...
def draw_home_page(): #dessine l'écran d'accueil
    ...
    diff_text_easy = WINNER_FONT.render('Facile : tapez 0',1,BLACK)
    WINDOW.blit(diff_text_easy,(300,200))
    diff_text_medium = WINNER_FONT.render('Moyen : tapez 1',1,BLACK)
    WINDOW.blit(diff_text_medium,(300,250))
    diff_text_hard = WINNER_FONT.render('Difficile : tapez 2',1,BLACK)
    WINDOW.blit(diff_text_hard,(300,300))
...
def play_mac():
    ...
    while run:  # boucle du jeu : tourne tant que le jeu n'est pas terminé
        ...
        #calcule le temps total passé en jeu pour pouvoir ensuite afficher le timer
        if not in_pause and not game_won:
        ...
        for event in pygame.event.get():
            ...
            # permet de quitter le jeu (met run à False quand l'utilisateur clique sur la croix ou échap ou sur le bouton quitter avec la souris)
            run = handle_quit(event, run)
            ...
            if (event.type == pygame.KEYDOWN and event.key == pygame.K_n) or interact_new_game(event,BUTTON_NEW_GAME):#permet de lancer une nouvelle partie en cliquant sur le bouton nouvelle partie ou en tapant la touche 'n'
                ...
                solved_grid = copy.deepcopy(grid)
                solve_sudoku_unique(solved_grid,0,0) #grille solution correspondant à la grille de la nouvelle partie
        ...
        draw_window(grid, current_case_selected_i, current_case_selected_j,original_coordinates,total_time,comments_grid,comments_activated,in_pause,game_won,music_activated)
